...
class DemoNode(Node):
    ...
    def sendcmd(self):
        ...
        time = self.get_clock().now().nanoseconds * 1e-9 - self.start
        base = BASE_RANGE * np.sin(time * FREQ)
        shoulder = SHOULDER_RANGE * np.sin(time * FREQ)
        wrist = WRIST_RANGE * (np.cos(time * FREQ) - 1)
        print([base, shoulder, wrist])
        self.cmdmsg.name         = ['one', 'two', 'three']
        self.cmdmsg.position     = [base, shoulder, wrist]
